	...
	for (arg = 1; (((opm = argv[arg][0]) - 2) | 2) == '+';)
	{
		...
		case 'd':
			...
			else if (!strcmp(optstr, "-dngsdk"))
			{
				...
				raw_processor.set_dng_host(dnghost);
			}
...
#define C raw_processor.imgdata.color
...
#define P2 raw_processor.imgdata.other
	...
	for (; arg < argc; arg++)
	{
		...
		if (use_mmap)
		{
			...
			close(file);
			if ((ret = raw_processor.open_buffer(iobuffer, st.st_size) != LIBRAW_SUCCESS))
			{
			...
			else
			{
				if (use_bigfile)
					...
					ret = raw_processor.open_file(argv[arg], 1);
				else
					ret = raw_processor.open_file(argv[arg]);
		...
		snprintf(outfn, sizeof(outfn), "%s.%s", argv[arg], OUT.output_tiff ? "tif" : (P1.colors > 1 ? "ppm" : "pgm"));
		...
		if (LIBRAW_SUCCESS != (ret = raw_processor.dcraw_ppm_tiff_writer(outfn)))
		...
		else
		{
			...
			cv::Mat cv_image(cv::Size(image->width, image->height), CV_16UC3, image->data, cv::Mat::AUTO_STEP);
			cv::cvtColor(cv_image, cv_image, CV_RGB2BGR);	//Convert RGB to BGR
			cv::imwrite("../data/cv_image.tif", cv_image);
			cv::Mat cv_image_gamma(cv::Size(image->width, image->height), CV_16UC3);
			cv_image.convertTo(cv_image_gamma, -1, 2.222, 45);
